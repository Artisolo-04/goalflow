...
before(async () => {
  const goalResult = await pool.query(
    `INSERT INTO goals (title) VALUES ('TEST: progress goal') RETURNING id`
  );
  goalId = goalResult.rows[0].id;

  const t1 = await pool.query(
    `INSERT INTO tasks (title, due_date, goal_id, completed) VALUES ('t1', '2026-01-01', $1, true) RETURNING id`,
    [goalId]
  );
  taskIds.push(t1.rows[0].id);

  const t2 = await pool.query(
    `INSERT INTO tasks (title, due_date, goal_id, completed) VALUES ('t2', '2026-01-01', $1, false) RETURNING id`,
    [goalId]
  );
  taskIds.push(t2.rows[0].id);

  const t3 = await pool.query(
    `INSERT INTO tasks (title, due_date, goal_id) VALUES ('t3', '2026-01-01', $1) RETURNING id`,
    [goalId]
  );
  ...
  const t4 = await pool.query(
    `INSERT INTO tasks (title, due_date, goal_id) VALUES ('t4', '2026-01-01', $1) RETURNING id`,
    [goalId]
  );
...
test('goal progress correctly averages task completion, weighting each task equally', async () => {
  ...
  const progress = Number(result.rows[0].progress);

  assert.equal(progress, 62.5);
});
